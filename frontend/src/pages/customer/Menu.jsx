...
// ⬇️ CustomerHome এ যেটা ছিল সেটাই ইউজ করলাম
...
export default function Menu() {
  ...
  useEffect(() => {
    let live = true;
    api.get("/menu")
      .then(res => { if (live) setItems(res.data || []); })
  ...
  const addToCart = (m) => {
    setCart(prev => {
      ...
      return [...prev, { id: m.id, name: m.name, price: Number(m.price), qty: 1, img }];
    });
    message.success(`${m.name} added to cart`);
  };

  const updateQty = (id, d) =>
    setCart(prev => prev.map(r => r.id === id ? { ...r, qty: Math.max(1, r.qty + d) } : r));
  ...
  const placeOrder = async () => {
    ...
    try {
      const payload = { items: cart.map(r => ({ menu_item_id: r.id, qty: r.qty })) };
            ...
            <>
              {cart.map(row => (
                <div key={row.id} className="flex items-center gap-3 mb-3">
                  <img src={row.img} alt={row.name} style={{ width: 56, height: 56, objectFit: "cover", borderRadius: 8 }} />
                  <div className="flex-1">
                    <div className="font-semibold">{row.name}</div>
                    <div className="text-sm text-[#737373]">৳ {row.price.toFixed(2)}</div>
                  </div>
                  <div className="flex items-center gap-2">
                    <Button size="small" onClick={() => updateQty(row.id, -1)}>-</Button>
                    <span>{row.qty}</span>
                    <Button size="small" onClick={() => updateQty(row.id, +1)}>+</Button>
                  </div>
                  <Button danger size="small" onClick={() => removeItem(row.id)}>Remove</Button>
                </div>
              ))}
              <hr />
              ...
              <Button
                type="primary"
                style={{ marginTop: 12, width: "100%", backgroundColor: "#FFB347", border: "none" }}
                onClick={placeOrder}
              >
                Place Order
              </Button>
